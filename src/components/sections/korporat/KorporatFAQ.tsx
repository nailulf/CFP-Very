...
export const KorporatFAQ: React.FC = () => {
  const { lang } = useLang();
  const t = translations[lang].korporat.faq;
          ...
          {t.items.map((item, i) => {
            ...
            return (
              <div key={item.q} className="bg-white rounded-2xl border border-[#E0EBF5] overflow-hidden">
                <button type="button" onClick={() => setOpen(isOpen ? null : i)} aria-expanded={isOpen} aria-controls={`korporat-faq-${i}`} className="w-full flex items-center justify-between gap-4 p-5 text-left">
                ...
                {isOpen && <p id={`korporat-faq-${i}`} className="px-5 pb-5 text-[14px] text-[#666666] leading-relaxed">{item.a}</p>}
              </div>
            );
          })}
        </div>
